...
- **Manual Screening Overhead:** Technical recruiters and hiring managers spend significant time manually reviewing resumes against lengthy job requirements.
- **Keyword Stuffing Exploits:** Naive keyword-matching ATS filters can be misled by repetition without semantic relevance.
- **Unstructured Technical Interviews:** Interviewers often default to generic trivia rather than targeted questions assessing candidate-specific gaps and project depth.

---

## Solution

The **AI Hiring Assistant** provides an automated evaluation pipeline that:
...
- 🧠 **Local Open-Source LLM Inference:** Generates executive summaries, interview guides, and roadmaps using quantized `Qwen2.5-0.5B-Instruct` on CPU (`torch.float16`) with zero external API costs.
...
| **Scoring Engine** | NumPy | Dense vector dot products & weighted hybrid ATS ensemble |
...
| **Automated Testing** | Pytest | 18 deterministic unit and integration tests |
